...
class NeuralNet {
    using MatrixfVec = std::vector<Matrixf>;
    ...
    // without biases
    Matrixf feedforward(Matrixf inputs) {
        // Sigmoid function
    ...
    void backpropagate(Matrix <float> inputs, Matrix <float> expected) {
        ...
        Matrixf results = feedforward (inputs);

        // Vectors of matrices holding error values of each layer 
        MatrixfVec errors(hWages.size());
        // Calculate the error of NN's output
        Matrixf outputErr = results - expected;
        // get the derivative of the sum value of outputs
        auto dValue = results.map(dSigmoid);
        // push the error maped by dValues to errors
        errors.back() = Matrixf::elementwise(outputErr, dValue);
        // Calculate total error
        outputErr = expected - results;
        // apply x^2/2 
        error = 0;
        outputErr.map(lSquares);
        // get the sum
        for (int i = 0; i < outputErr.getSize().first; ++i)
            error += outputErr[i][0];
        ...
        for (int i = hLayers-1; i >= 0; --i) {
            ...
            errors[i] = Matrixf::elementwise(errors[i], dValue);
        }
